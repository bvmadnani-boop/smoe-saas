...
export default async function ReviewDetailPage({
  ...
  params: Promise<{ orgId: string; reviewId: string }>
}) {
  const { orgId, reviewId } = await params
  ...
  // Re-fetch fresh data for re-inject
  const [
    { data: ncs },
    { data: activePolicy },
    { data: allRisks },
    ...
    supabase.from('nonconformities').select('id, title, severity, status, category, detected_at, ocap_deadline')
    ...
    supabase.from('risks').select('id, title, type, score, status, treatment, category')
      .eq('organization_id', orgId).eq('is_active', true).order('score', { ascending: false }),
    supabase.from('audit_plans').select('id, title, process_key, status, audit_date, trimestre')
      .eq('organization_id', orgId).order('audit_date', { ascending: true }),
    supabase.from('swot_items').select('id, quadrant, content, is_active').eq('organization_id', orgId),
    supabase.from('pestel_items').select('id, dimension, content, impact, is_active').eq('organization_id', orgId),
    supabase.from('interested_parties').select('id, name, group_key, needs, expectations, is_active').eq('organization_id', orgId),
    supabase.from('teachers').select('id, full_name, sup2i_validated').eq('organization_id', orgId),
    supabase.from('schedules').select('id, teacher_id, teachers(id, sup2i_validated)').eq('organization_id', orgId),
    supabase.from('management_reviews')
      .select('id, title, review_date, status').eq('organization_id', orgId)
      .eq('status', 'realise').neq('id', reviewId)
      .order('review_date', { ascending: false }).limit(1).single(),
  ])
  ...
  const data = {
    ...
    totalTeachers,
    validatedTeachers,
    sup2iPct:          totalTeachers > 0 ? Math.round((validatedTeachers / totalTeachers) * 100) : 0,
